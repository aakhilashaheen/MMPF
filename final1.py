...
class BPF(object):
    ...
    def fit(self, ratings, num_steps):
...
def test(hyperparams):
    idx = [(u, i) for u in range(hyperparams['num_users']) for i in range(hyperparams['num_items'])]
    random.shuffle(idx)
    raw_ratings = Poisson(3.).sample((hyperparams['num_users'], hyperparams['num_items']))
    ratings = [(u, i, raw_ratings[u, i]) for u, i in idx[:40]]
        
    mmpf = MMPF(hyperparams)
    bpf = BPF(hyperparams)
    mmpf_losses = mmpf.fit(ratings, num_steps=10)
    bpf_losses = bpf.fit(ratings, num_steps=10)
...
def main():
    ...
    bpf_losses1, mmpf_losses1 = test(hyperparams)
    print(bpf_losses1)
    print(mmpf_losses1)
    ...
    hyperparams = {}
    hyperparams['a_u'] = hyperparams['b_u'] = hyperparams['a_i'] = hyperparams['b_i'] = hyperparams['a_c'] = hyperparams['b_c'] = hyperparams['c_u'] = hyperparams['c_i'] = torch.tensor(1.)
    hyperparams['context_conc'] = 6.
    hyperparams['num_users'] = 100
    hyperparams['num_items'] = 200
    hyperparams['num_nonmissing'] = 50
    hyperparams['num_latents'] = 8
    hyperparams['num_contexts'] = 8
    hyperparams['num_context_latents'] = 8

    bpf_losses2, mmpf_losses2 = test(hyperparams)
    print(bpf_losses2)
    print(mmpf_losses2)
