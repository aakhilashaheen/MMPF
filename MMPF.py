...
class BPF(object):
    ...
    def _model(self, ratings, num_latents, hyperparams=None):
    ...
    def _guide(self, ratings, num_latents, hyperparams=None):
        if hyperparams is None:
            hyperparams = self.hyperparams
    ...
    def fit(self, ratings, num_latents, hyperparams=None, num_steps=2000):
        if hyperparams is None:
        ...
        for s in tqdm(range(num_steps)):
            svi.step(ratings, num_latents, hyperparams)
...
class MMPF(BPF):
    ...
    def _model(self, ratings, num_latents, num_contexts, num_context_latents, context_conc, hyperparams=None):
        if hyperparams is None:
            hyperparams = self.hyperparams
        ...
        context_prob_dist = dist.Dirichlet(context_conc * torch.ones(num_contexts))
        ...
        with pyro.plate('user_contexts', num_contexts):
            with pyro.plate('user_context_latents', num_context_latents):
                user_context_latents = pyro.sample('user_context_latents', context_dist)

        with pyro.plate('item_contexts', num_contexts):
            with pyro.plate('item_context_latents', num_context_latents):
        ...
        for i in pyro.plate('ratings', ratings.num_nonmissing):
            user, item, rating = next(ratings)
    ...
    def _guide(self, ratings, num_latents, num_contexts, num_context_latents, context_conc, hyperparams=None):
        if hyperparams is None:
            hyperparams = self.hyperparams

        q_user_context_conc = pyro.param('q_user_context_conc',
                                         context_conc * torch.ones(num_context_latents))
        ...
        for i in pyro.plate('ratings', ratings.num_nonmissing):                   
            user, item, rating = next(ratings)
            q_zu = pyro.param('q_zu_{},{}'.format(user, item), torch.ones(num_contexts),
                              constraint=positive)
            q_zi = pyro.param('q_zi_{},{}'.format(user, item), torch.ones(num_contexts),
                              constraint=positive)
            z_u = pyro.sample('user_context', dist.Categorical(q_zu))
            z_i = pyro.sample('item_context', dist.Categorical(q_zi))

    def fit(self, ratings, num_latents, num_contexts, num_context_latents, context_conc, hyperparams=None, num_steps=2000):
        if hyperparams is None:
            if self.hyperparams is None:
                raise ValueError, 'Hyperparameters not provided!'
            else:
        ...
        svi = SVI(self._model, self._guide, Adam(), loss=TraceGraph_ELBO())
        for s in tqdm(range(num_steps)):
            svi.step(ratings, num_latents, hyperparamsself, ratings, num_latents, num_contexts, num_context_latents, context_conc, hyperparams=None)
