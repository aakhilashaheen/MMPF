...
class BPF(object):
    ...
    def _model(self, ratings):
        ...
        with pyro.plate('users_loop', hyperparams['num_users']):
            ...
            with pyro.plate('user_latents_loop', hyperparams['num_latents']):
        ...
        with pyro.plate('items_loop', hyperparams['num_items']):
            ...
            with pyro.plate('item_latents_loop',  hyperparams['num_latents']):
    ...
    def _guide(self, ratings):
        for u in pyro.plate('users_loop', hyperparams['num_users']):
            ...
            # Sample latents
            for l in pyro.plate('user_latents_loop', hyperparams['num_latents']):
        ...
        for i in pyro.plate('items_loop', hyperparams['num_items']):
            ...
            # Sample latents
            for l in pyro.plate('item_latents_loop', hyperparams['num_latents']):
...
class MMPF(object):
    ...
    def _model(self, ratings):
        ...
        for j in pyro.plate('ratings', self.hyperparams['num_nonmissing']):
            ...
            lam = user_latents[user, :] @ item_latents[item, :] + \
                user_context_latents[z_u[j], :] @ item_context_latents[z_i[j], :]
...
bpf = BPF(hyperparams)
optim = Adam({'lr': 0.1, 'betas': [0.8, 0.99]})
svi = SVI(bpf._model, bpf._guide, optim, loss=TraceEnum_ELBO())
losses = []
